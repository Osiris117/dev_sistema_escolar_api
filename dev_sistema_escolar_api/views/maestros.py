...
class MaestrosView(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        #Solo maestros o administradores pueden modificar perfiles de maestros
        if not request.user.groups.filter(name__in=['maestro', 'administrador']).exists():
            return Response({"details": "No autorizado para modificar maestros"}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, *args, **kwargs):
        maestro = get_object_or_404(Maestros, id=request.GET.get("id"))
        #Solo administradores pueden eliminar maestros
        if not request.user.groups.filter(name='administrador').exists():
            return Response({"details": "No autorizado para eliminar maestros"}, status=status.HTTP_403_FORBIDDEN)
        try:
            maestro.user.delete()
