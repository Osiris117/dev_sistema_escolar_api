...
class AlumnosView(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        # Validamos permisos: maestros/administradores o el propio alumno
        puede_modificar = False
        if request.user.groups.filter(name__in=['maestro', 'administrador']).exists():
            puede_modificar = True
        elif alumno.user == request.user:
            puede_modificar = True

        if not puede_modificar:
            return Response({"details": "No autorizado para modificar este alumno"}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, *args, **kwargs):
        alumno = get_object_or_404(Alumnos, id=request.GET.get("id"))
        #Solo maestros o administradores pueden eliminar alumnos
        if not request.user.groups.filter(name__in=['maestro', 'administrador']).exists():
            return Response({"details": "No autorizado para eliminar alumnos"}, status=status.HTTP_403_FORBIDDEN)
        try:
            alumno.user.delete()
